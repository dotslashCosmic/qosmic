...
use crate::constants::*;
use crate::qosmic;
use crate::utils::{self, key_as_u128, key_as_u32, key_as_u64};
use log::debug;
...
pub fn z_func_factory_internal(binary_input: &[u8], _key_input: &[u8]) -> BitGenerator {
    let mut internal_csprng_state: u128 = key_as_u128() ^ CONST as u128;
    ...
    Box::new(move || {
        if block_idx >= current_output_block.len() {
            ...
            block_idx = 0;
            internal_csprng_state = internal_csprng_state.wrapping_add(counter as u128).wrapping_mul(key_as_u128());
            internal_csprng_state &= MASK_128;
            let mut derived_material_state = internal_csprng_state;
            for i in 0..16 {
                derived_material_state = derived_material_state.wrapping_add(counter as u128).wrapping_mul(key_as_u128());
                ...
                derived_material_state ^= (key_as_u128() << 32).wrapping_add(CONST as u128);
            ...
            for i in 0..13 {
                ...
                derived_material_state ^= (key_as_u128() >> (i % 2 * 64)) ^ (CONST as u128);
            ...
            for i in 0..5 {
                p_array[i] = derived_u64s[derived_idx].wrapping_add(COEFFS[i % 5]).wrapping_mul(MAGIC);
                p_array[i] |= 1;
